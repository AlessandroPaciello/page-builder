...
| `.bmad/stories/active|completed|backlog` | `_bmad-output/implementation-artifacts/{story_key}.md` — status tracked via the story file's own `Status` field, not by folder. Don't invent active/completed/backlog subfolders unless the user asks for them. |
