...
import { VerdictCollector, publishReport, type ComponentReport } from "../shared/component-report";
...
import { resolvePartAliases } from "../extract/recipe-schema";
...
/**
 * Gate del regime design→codice (Story 2.6, Task 8; per componente dalla
 * Story 2.8 parte B); l'entry CLI è `src/cli/gates-render.ts`:
 *
 *   gates:render
...
function runUiSuite(): A11ySuiteResult {
  ...
  const suite = spawnSync("pnpm", ["test"], { cwd: PATHS.uiPackageRoot, stdio: "inherit" });
  const spawnError =
...
export async function runGates(deps: GatesDeps = defaultGatesDeps()): Promise<ComponentReport> {
  ...
  if (components.length === 0 && malformed.length === 0) {
    global.red("ricette committate", "Nessuna ricetta committata in data/recipes — i gate non hanno componenti da coprire.");
  }
...
/**
 * Corpo del CLI senza `process.exit`: argomenti già parsati → gate → report (terminale,
