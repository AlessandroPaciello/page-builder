...
import { DEFAULT_PATHS, formatDiff, runAdopt, type AdoptPaths } from "./adopt-command";
...
function outlineSnapshot(): LibrarySnapshot {
  const f = FixtureSchema.parse(JSON.parse(readFileSync(resolve(PATHS.recipesDir, "badge.fixture.json"), "utf8")));
...
describe("adopt:variant (CLI e comando)", () => {
  ...
  it("snapshot malformato (--snapshot) → exit 1 nominativo, non SyntaxError grezzo", async () => {
    ...
    expect(await runAdopt({ component: "Badge", yes: false, snapshotPath }, { paths })).toBe(1);
  ...
  it("binding malformato (JSON non leggibile) → exit 1 nominativo col percorso", async () => {
    ...
    expect(await runAdopt({ component: "Badge", yes: false, snapshotPath: join(paths.designsDir, "..", "snapshot.json") }, { paths })).toBe(1);
  ...
  it("file sorgente mancante → errore prima di qualunque lettura di Penpot", async () => {
    ...
    await expect(runAdopt({ component: "Badge", yes: false }, { paths, callTool })).rejects.toThrow(/File non leggibile.*badge\.binding\.json/);
  ...
  it("senza --yes: stampa il diff dei cinque file, exit 0, nessuna scrittura (--snapshot)", async () => {
    ...
    expect(await runAdopt({ component: "Badge", yes: false, snapshotPath }, { paths })).toBe(0);
  ...
  it("--yes: scrive i cinque file; fingerprint e binding validi sui file generati; secondo run = nulla da adottare", async () => {
    ...
    expect(await runAdopt({ component: "Badge", yes: true }, { paths, callTool })).toBe(0);
  ...
  it("errore (valore non valido su un asse option): exit 1, nessuna scrittura", async () => {
    ...
    expect(await runAdopt({ component: "Badge", yes: true }, { paths, callTool })).toBe(1);
    expect(contents(paths)).toEqual(before);
    expect(err.mock.calls.map((call: unknown[]) => String(call[0])).join("\n")).toContain('"Bad Value"');
  });
});
