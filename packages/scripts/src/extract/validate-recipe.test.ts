...
describe("validateRecipe", () => {
  ...
  it("valida pulite le coppie fixture+ricetta committate di ogni contratto (AC #1)", () => {
    ...
    const names = readdirSync(PATHS.recipesDir)
      .filter((entry) => entry.endsWith(".recipe.json"))
      .map((entry) => entry.replace(/\.recipe\.json$/, ""))
      .sort();
    expect(names.length).toBeGreaterThan(0);
    for (const name of names) {
      const fixture = JSON.parse(readFileSync(resolve(PATHS.recipesDir, `${name}.fixture.json`), "utf8")) as unknown;
      const recipe = JSON.parse(readFileSync(resolve(PATHS.recipesDir, `${name}.recipe.json`), "utf8")) as unknown;
      const judgment = JSON.parse(readFileSync(resolve(PATHS.recipesDir, `judgments/${name}.json`), "utf8")) as unknown;
